	...
	private boolean obtenerRevisores() {
		...
		if (trackRevisoresMio.size() > 0) {
			for (int i = 0; i < trackRevisoresMio.size(); i++) {
				...
				for (PedirColaboradorDTO si : revisores) {
					
					trackRevisores = model.obtenerTrack(si.getEmailUsuario());
					for (PedirColaboradorDTO si2 : trackRevisores) {
					
						if (si2.getIdTrack() == trackRevisoresMio.get(i).getIdTrack()) {
							decision = model.obtenerDeicision(si.getEmailUsuario(), idArticulo);
