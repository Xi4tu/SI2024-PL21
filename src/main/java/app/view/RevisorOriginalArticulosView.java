...
public class RevisorOriginalArticulosView {
	// Crear las variables de la vista
	private JFrame frame;
	private JPanel contentPane;
	private JList<RevisionArticuloRevisorDTO> listArticulos;
	private JList<RevisionArticuloRevisorDTO> listArticulosPendientes;
	...
	public RevisorOriginalArticulosView() {
		initialize();
	}
	...
	private void initialize() {
		...
		// ---------------------------------------------------------------------
		// Panel Izquierdo con JTabbedPane
		// ---------------------------------------------------------------------
		JTabbedPane tabbedPane = new JTabbedPane();
		JPanel panelAsignados = new JPanel(new BorderLayout());
		panelAsignados.setBorder(new TitledBorder("Artículos"));
		listArticulos = new JList<>();
		listArticulos.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		JScrollPane scrollAsignados = new JScrollPane(listArticulos);
		panelAsignados.add(scrollAsignados, BorderLayout.CENTER);
		tabbedPane.addTab("Aceptados", panelAsignados);

		JPanel panelPendientes = new JPanel(new BorderLayout());
		panelPendientes.setBorder(new TitledBorder("Artículos Pendientes"));
		listArticulosPendientes = new JList<>();
		listArticulosPendientes.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		JScrollPane scrollPendientes = new JScrollPane(listArticulosPendientes);
		panelPendientes.add(scrollPendientes, BorderLayout.CENTER);
		tabbedPane.addTab("Pendientes", panelPendientes);
		listArticulosPendientes.setEnabled(false);

		tabbedPane.setPreferredSize(new Dimension(250, 0));
	...
	public JList<RevisionArticuloRevisorDTO> getListArticulosPendientes() {
		return listArticulosPendientes;
	}
